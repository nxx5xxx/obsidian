...
> clone과 list로 처리 하지말고 차라리 저 안에서 계산을 하여 set에 저장을 하는게 나은가  
...
> 그렇게 1번플레이어와 2번플레이어의 경우의 수 값을 더해서 출력 하는것으로 답을 구하였다
...
>	static int[] d = new int[16];  
	...
	/* n\*2테스트  
// 4에서 막힌다 앞에는 피보나치인데 n-4부터 짜지질않는다 접근법이 잘못된건지 점화식을 못세우  고 있는건지 모르겠다 1 : 1, 2: 2,  
...
> 미완성으로 제출한 코딩테스트  
> Dynamic progarmming 이라는 풀이법을 사용해볼 수 있어서 좋았다  
> 단, 아직 익숙하지 않다 이정도로 심화된 문제를 풀어본 적이 없어  
> 동적 프로그래밍을 잘 설계하지 못한다는것을 깨닫게 될 수 있엇고  
> 시간이 충분하다면 다시 풀어보고 싶다.
> switch()문을 이용하여 1~15로 나눌것 처럼 하였지만  
> 1~15 사이에도 특정한 점화식을 이용하면 될것같은 느낌적인 느낌이 들어  
> 더 풀어보고 싶은 코테문제 면접준비를 해야해 면접이 끝난 후 한번 더 정리를 해야겠다.